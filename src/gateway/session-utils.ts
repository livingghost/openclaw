...
export async function prewarmSessionUsageCache(params: {
  ...
  // Yield to the event loop so the synchronous scan below does not
  // block gateway startup when called fire-and-forget.
  await Promise.resolve();

  const { cfg, log } = params;
  // Always call the setter so removing the config key restores the default
  // limit on in-process gateway restarts (module state persists across restarts).
