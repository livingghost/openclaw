...
describe("loadGatewayPlugins", () => {
  ...
  test("forwards structured plugin subagent options to gateway agent methods", async () => {
    ...
    // clientTools, disableTools, and streamParams are intentionally NOT forwarded
    // because AgentParamsSchema does not include these fields yet.
    const dispatched = getLastDispatchedRequest();
    expect(dispatched?.method).toBe("agent");
    expect(dispatched?.params?.sessionKey).toBe("s-structured");
    expect(dispatched?.params?.message).toBe("extract memories");
  ...
  test("does not forward unsupported tool-control fields to gateway agent methods", async () => {
    const serverPlugins = await importServerPluginsModule();
    const runtime = await createSubagentRuntime(serverPlugins);
    serverPlugins.setFallbackGatewayContext(createTestContext("unsupported-fields-run"));

    await runtime.run({
      sessionKey: "s-unsupported-run",
      message: "test",
      clientTools: [{ name: "test_tool", description: "test", parameters: { type: "object", properties: {} } }],
  ...
  test("does not forward unsupported tool-control fields via enqueue", async () => {
    const serverPlugins = await importServerPluginsModule();
    const runtime = await createSubagentRuntime(serverPlugins);
    serverPlugins.setFallbackGatewayContext(createTestContext("unsupported-fields"));

    await runtime.run({
      sessionKey: "s-unsupported",
      message: "test",
      clientTools: [
        {
          name: "test_tool",
